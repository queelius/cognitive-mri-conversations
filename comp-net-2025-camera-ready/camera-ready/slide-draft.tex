...
\begin{frame}{From Log to a ``Cognitive MRI''}
    ...
    \begin{columns}[T]
        ...
        \end{tikzpicture}\\
        \tiny \textbf{Embed}\\ \textbf{\& Link}\\ \textbf{($\theta$)}
    ...
    \begin{block}{Building the Network}
        \footnotesize % Smaller font to ensure it fits
        \textbf{Nodes:} Conversations as embeddings (LLM-based vectors). \quad
        \textbf{Edges:} Cosine similarity $>$ threshold $\theta$.\\
        Distance in \textbf{Time} $\neq$ Distance in \textbf{Thought}. The network reconnects what the timeline separates.
    \end{block}
