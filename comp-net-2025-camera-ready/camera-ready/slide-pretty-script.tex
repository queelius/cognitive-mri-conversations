...
``Is that actually true? That's what we wanted to test. Today I'll show you one case study.''

\slide{Slide 3: The Big Picture: Externalized Cognition}

``We're using a framework called Distributed Cognition. The idea is simple: thinking doesn't just happen in your head. It happens between you and your tools.''
...
``Now, what gets saved? Usually just the product. The final draft. Clean and polished.''

``But the messy part---the exploring, the dead ends---that usually disappears.''

``Think about fixing a bug. Twenty rounds of trying things. False starts. Then finally it clicks. The commit message? One line.''

``Or a mathematician. Notebooks full of wrong turns. We only see the final proof.''
...
``Start with a chat history. It's just a timeline. January: Python bug. February: banana bread. March: more debugging. April: ethics.''

``We turn each conversation into a vector. 768 dimensions. Similar topics land near each other in that space.''

``Then we connect by similarity, not by time.''

``What happens? Those two coding chats---months apart---snap together. Banana bread floats off on its own. Philosophy clusters somewhere else.''

``The key insight: close in time doesn't mean close in thought. The network reconnects what the timeline splits apart.''
...
``We take the average of your messages, the average of the AI's, then blend them 2-to-1 in your favor. One vector per conversation.''

``That gives us two knobs to turn. Alpha: how much to weight user versus AI. Theta: how similar two conversations need to be before we connect them.''
...
``For the threshold: there's a tipping point around 0.875. Below that, everything connects. It's a mess. Above that, it falls apart into fragments.''
...
``It's not uniform. About a quarter is this dense core---general stuff that connects everywhere. The edges are specialized. And the average path? About 6 hops between any two chats.''
...
``Theory---math, ML concepts, philosophy---clusters tightly. Clustering coefficient around 0.58. Small-world structure.''
