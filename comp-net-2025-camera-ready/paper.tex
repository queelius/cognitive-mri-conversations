...
\begin{document}
\mainmatter
\title{Cognitive MRI of AI Conversations: A Single-User Case Study}
...
We introduce three key concepts: A \emph{cognitive MRI} reveals latent thought structure in conversational data through network visualization, analogous to medical MRI revealing brain structure. \emph{Cognitive structure} refers to knowledge organization patterns manifested through conversation---the topology of concepts and domains. \emph{Topological maps} are graph representations exposing knowledge navigation geometry through nodes (conversations) and edges (semantic similarities).
...
\begin{itemize}
    \item Nine weight ratios: 100:1, 4:1, 2:1, 1.618:1, 1:1, 1:1.618, 1:2, 1:4, 1:100 (user:AI)
    \item Seven similarity thresholds: 0.8, 0.825, 0.85, 0.875, 0.9, 0.925, 0.95
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{./images/threshold_evolution_clean.png}
\caption{Network metrics across similarity thresholds. Critical phase transition at $\theta \approx 0.875$: below this, the network maintains over 90\% connectivity; above it, catastrophic fragmentation occurs.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{./images/weight_ratio_analysis_clean.png}
\caption{Weight ratio optimization at $\theta = 0.9$. The 2:1 user:AI configuration maximizes modularity (0.750) with 15 distinct communities.}
...
Figure~\ref{fig:semantic_boundary} illustrates how threshold variation reveals semantic boundaries. At $\theta=0.9$ (left), a niece conversation cluster remains isolated from the main network. At $\theta=0.875$ (right), this cluster connects through philosophical bridge conversations, illustrating the 2.5\% semantic distance between technical and peripheral conversational contexts.
...
As shown in Table~\ref{tab:2d_summary}, the independence of threshold effects (controlling connectivity) from weight ratio effects (controlling organization) suggests these parameters tune orthogonal aspects of network structure. Threshold acts as a \emph{semantic filter} determining which connections exist, while weight ratio provides \emph{semantic perspective} determining how knowledge organizes within the connected structure.
...
Three distinct bridge types emerge, extending structural bridge classifications \cite{granovetter1973,burt1992} with semantic characterization. First, \emph{evolutionary bridges} are high-degree nodes that expand across domains through topic drift. For example, \url{geometric-mean-calculation} evolved from geometric means into probability theory, neural networks, and tokenization (degree=61, betweenness=45467). Second, \emph{integrative bridges} are moderate-degree nodes that deliberately synthesize domains, such as \url{mcts-code-analysis-suggestions} which integrates AI theory with software engineering (degree=10, betweenness=36909). Third, \emph{pure bridges} are low-degree critical links like \url{compile-cuda-program-linux} (degree=2, betweenness=9775). Figure~\ref{fig:bridge_zoom} illustrates these distinct bridging mechanisms in the network structure.

\begin{figure}[!h] %tbp]
\centering
\includegraphics[width=3.5in]{./images/bridge-better.png}
\caption{Bridge conversations facilitating cross-domain knowledge transfer. Three bridge types are shown: \emph{evolutionary bridge} (geometric-mean) connecting ML/AI and Stats/Prob communities, \emph{integrative bridge} (mcts-code) synthesizing multiple domains, and \emph{pure bridge} (cuda-program-linux) providing minimal critical links between Programming Concepts and Practical Programming.}
...
Through a single-user case study, we presented a novel application of complex network analysis to AI conversations, revealing distinct knowledge communities and bridge conversations. Our comprehensive ablation study established that a 2:1 user:AI weighting ratio ($\alpha = 2$) optimally balances user intent with AI semantic contributions for this dataset, achieving maximum modularity (0.750) at similarity threshold $\theta = 0.9$. The heterogeneous network topology reflects patterns in AI-assisted topic exploration, with bridges facilitating movement between specialized domains. This \emph{cognitive MRI} methodology provides a framework for understanding and enhancing AI-assisted knowledge work as these systems become increasingly integrated into human cognition.
