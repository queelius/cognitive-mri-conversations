...
\author{
  ...
  \textsuperscript{1} Department of Computer Science, Southern Illinois University Edwardsville, Edwardsville, IL, USA \\[3pt]
...
In prior work, we introduced a \emph{cognitive MRI} methodology that transforms such archives into semantic similarity networks, where conversations become nodes and edges connect semantically related pairs~\citep{towell2025cognitive}. Analyzing a single user's 449 ChatGPT conversations (filtered at similarity threshold $\theta = 0.9$), we identified 15 knowledge communities, heterogeneous network topology (hub-and-spoke vs.\ tree-like structures), and three distinct bridge conversation types that facilitate cross-domain knowledge transfer. That analysis, however, treated the network as a static snapshot --- collapsing 29~months of knowledge exploration into a single graph.
...
\begin{itemize}
  ...
  \item \textbf{Bridge persistence}: once a conversation achieves bridge status (top-5\% betweenness centrality), it maintains that role indefinitely.
...
We fit this relationship using ordinary least squares regression on the log-transformed data, reporting the exponent $\gamma$, coefficient of determination $R^2$, and $p$-value. We exclude early months with fewer than 4 connected nodes where metrics are unstable.
...
Community tracking across 29 snapshots reveals 35 unique tracked communities, of which 15 survive to the final snapshot (Table~\ref{tab:community_events}). The lifecycle analysis recorded 196 continuation events, 35 births, and 20 deaths. No merge or split events were detected at the Jaccard threshold of $J = 0.3$, suggesting that communities in this network grow and dissolve rather than recombining.
...
The remaining four bridges enter the connected network in October--November 2024, coinciding with the expansion of the giant component during the Reasoning model era. Once established, each maintains a stable betweenness centrality level: \texttt{mcts-code-analysis-suggestions} and \texttt{loss-in-llm-training} stabilize around 0.35, \texttt{algotree-generate-unit-tests-flattree} around 0.30, and \texttt{compile-cuda-program-linux} around 0.09--0.12.
...
The bridge conversations span 2--5 neighbor communities each. The \texttt{geometric-mean-calculation} bridge consistently connects 4 communities, confirming its role as a multi-domain integrator. In contrast, \texttt{compile-cuda-program-linux} connects exactly 2 communities throughout its tracked lifetime, exemplifying the ``pure bridge'' type: a minimal but critical link between domains.
...
The GPT-4 and GPT-4.5 eras contain too few connected conversations for meaningful structural analysis (15 and 2, respectively), reflecting their limited availability during the observation period.
...
\textbf{Model era confounds.} Differences between model era sub-networks may reflect temporal trends (topic interests change over time), model capabilities, or both. The small sample sizes for GPT-4 and GPT-4.5 eras preclude meaningful comparison.
...
These findings demonstrate that personal knowledge exploration through conversational AI is not random accumulation but follows lawful growth dynamics. The densification law quantifies how knowledge becomes increasingly interconnected; preferential attachment reveals the balance between deepening existing interests and exploring new ones; community tracking shows how knowledge domains crystallize early and persist; bridge dynamics illuminate how cross-domain connections form and stabilize.

As conversational AI becomes an increasingly integral part of knowledge work, understanding the temporal dynamics of human-AI knowledge exploration will become essential. Our methodology provides a template for studying these dynamics at scale, and our findings offer a first empirical characterization of how one person's knowledge landscape evolves through sustained AI interaction.
